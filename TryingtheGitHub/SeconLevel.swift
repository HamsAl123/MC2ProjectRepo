...
struct SecondLevel: View {
    ...
    var body: some View {
        ZStack{
            ...
            .overlay(
                Group {
                    if isShowingPopUp {
                        GoodjobPopUpView2(isShowingPopUp: $isShowingPopUp)
                    }
                    if !isShowingPopUp1 {
                        WeakPasswordPopUpView2(isShowingPopUp: $isShowingPopUp1)
                    }
                }
            )//end of overlay
        }
    ...
    func isValidPassword(_ password: String) -> Bool {
        let passwordRegex = "^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])[A-Za-z0-9\\W_]{5,}$"
        let passwordPredicate = NSPredicate(format: "SELF MATCHES %@", passwordRegex)
        return passwordPredicate.evaluate(with: password)
    }
}

struct GoodjobPopUpView2: View {
...
struct WeakPasswordPopUpView2: View {
    @Binding var isShowingPopUp: Bool
